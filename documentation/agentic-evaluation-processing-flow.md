...
```json
{
  ...
  "transcript_s3_key": "processed/abc123/sub-98765/transcript.json",
...
- All string fields must be non-empty (min_length=1): `submission_id`, `user_id`, `s3_file_key`, `transcript_s3_key`, `presentation_title`
- `vector_store_location` defaults to empty string (optional, empty when embeddings disabled)
- `chunk_count` must be ≥ 0 (default: 0, zero when embeddings disabled)
...
### 6.1 Submission Metadata Retrieval

Before building the PDF, the Session Supervisor gathers submission metadata for the report header:

1. **DynamoDB query** — retrieves `description`, `original_file_name`, `upload_date` from the submissions table
2. **Cognito user lookup** — retrieves the user's display name and email using `cognito-idp:AdminGetUser`
   - Resolves the user pool via `COGNITO_USER_POOL_NAME` environment variable (lists pools to find the ID)
   - Or uses `COGNITO_USER_POOL_ID` directly if set
   - Name resolution order: `name` → `given_name + family_name` → `given_name` → `email` → `user_id`
   - Formats as "Name (email)" (e.g., "Michael Geiser (mgeiser@mgeiser.net)")
   - Falls back gracefully to user_id if Cognito lookup fails

### 6.2 PDF Construction

The Report Generator uses ReportLab to produce a structured PDF coaching report with page numbers in the footer of every page.

**Report Header (Page 1, top):**
- Report title: "Presentation Coaching Report"
- User: Name and email from Cognito (e.g., "Michael Geiser (mgeiser@mgeiser.net)")
- Presentation Title
- Description (if provided)
- Presentation File (original file name)
- Uploaded: Date formatted as "June 22, 2026 12:15 ET" (UTC converted to Eastern Time)

**Sections:**
1. **Executive Summary** (Page 1) — LLM-generated narrative coaching summary containing:
   - One-sentence coaching diagnosis
   - Score interpretation (what the number means in coaching terms)
   - Strengths as a flowing narrative paragraph (not bullets)
   - The 2-3 highest-leverage improvements
   - A concrete next-practice target/coaching assignment
   
   The summary is generated by calling Claude Sonnet with a coaching-expert system prompt. If the LLM call fails, a basic score-only fallback is used.

2. **Per-Dimension Detailed Feedback** (starts on new page) — For each evaluated dimension:
   - Dimension display name and score (e.g., "Executive Presence (Score: 5.2/10.0)")
   - Findings (with severity, category, suggestion)
   - Strengths (bullet list)
   - Areas for Improvement (bullet list)

3. **Overall Coaching Assessment** (starts on new page) — Narrative assessment based on score range:
